...
export function QuizRunner({
  ...
  async function submit() {
    ...
    let answer: { kind: "mcq" | "true_false"; choiceId: string } | { kind: "numeric"; value: number };
      ...
      {question.kind === "numeric" && (
        <div className="mb-4">
          ...
          {question.numericTolerance && (
            <p className="mt-1 text-xs text-neutral-400">{question.numericTolerance}</p>
          )}
        </div>
      )}

      {question.hint && !result && <p className="mb-4 text-xs text-neutral-500">💡 {dict.hint}: {question.hint}</p>}

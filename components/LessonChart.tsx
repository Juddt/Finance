  ...
  return (
    <ChartFrame caption={caption}>
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full" role="img" aria-label={caption}>
        ...
        <text x={(WIDTH - PADDING_LEFT - PADDING_RIGHT) / 2 + PADDING_LEFT} y={HEIGHT - 4} fontSize={11} textAnchor="middle" fill="currentColor" opacity={0.6}>
          {chart.xLabel[locale]}
        </text>
        <text x={12} y={PADDING_TOP + 8} fontSize={11} textAnchor="start" fill="currentColor" opacity={0.6}>
          {chart.yLabel[locale]}
        </text>
      </svg>
    ...
    <ChartFrame caption={caption}>
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full" role="img" aria-label={caption}>
        ...
        {chart.bars.map((b, i) => {
          ...
          return (
            <g key={i}>
              <rect x={cx - barWidth / 2} y={barY} width={barWidth} height={Math.max(barHeight, 1)} fill={PALETTE[i % PALETTE.length]} rx={2} />
              <text x={cx} y={HEIGHT - PADDING_BOTTOM + 16} fontSize={10} textAnchor="middle" fill="currentColor" opacity={0.7}>
                {b.label[locale]}
              </text>
              <text x={cx} y={b.value >= 0 ? barY - 4 : barY + barHeight + 12} fontSize={10} textAnchor="middle" fill="currentColor" opacity={0.85}>
...
function ScatterChart({ chart, locale, caption }: { chart: Extract<LessonChartSpec, { kind: "scatter" }>; locale: "fr" | "en"; caption?: string }) {
  ...
  return (
    <ChartFrame caption={caption}>
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full" role="img" aria-label={caption}>
        <AxisLines xScale={xScale} yScale={yScale} xMin={xMin} xMax={xMax} yMin={yMin} yMax={yMax} />
        {trendPath && <path d={trendPath} fill="none" stroke="#d97706" strokeWidth={2} strokeDasharray="5 4" />}
        {chart.points.map((p, i) => (
          <circle key={i} cx={xScale(p.x)} cy={yScale(p.y)} r={4} fill="#2563eb" fillOpacity={0.75} />
        ))}
        <text x={(WIDTH - PADDING_LEFT - PADDING_RIGHT) / 2 + PADDING_LEFT} y={HEIGHT - 4} fontSize={11} textAnchor="middle" fill="currentColor" opacity={0.6}>
          {chart.xLabel[locale]}
        </text>
...
function Legend({ items }: { items: { label: string; color: string }[] }) {
  return (
    <div className="mt-1 flex flex-wrap justify-center gap-x-4 gap-y-1 text-xs text-neutral-600 dark:text-neutral-300">
      {items.map((item, i) => (
...
function ChartFrame({ children, caption }: { children: React.ReactNode; caption?: string }) {
  return (
    <figure className="my-2 rounded-xl border border-black/10 bg-neutral-50 p-3 dark:border-white/10 dark:bg-neutral-900">
      {children}
      {caption && <figcaption className="mt-1 text-center text-xs text-neutral-500">{caption}</figcaption>}
    </figure>
  );
}

...
export const m12SeriesTemporelles: LessonContent = {
  ...
  example: {
    fr: "Modéliser la volatilité quotidienne d'un indice actions avec un GARCH(1,1) : après un choc de marché (un grand mouvement de prix), le modèle prédit une volatilité élevée les jours suivants (clustering), puis une décroissance progressive vers la volatilité de long terme — un comportement que suppose implicitement tout desk qui ajuste ses couvertures dynamiquement après un choc, mais qu'un modèle à volatilité constante ne capture jamais.",
    en: "Modeling an equity index's daily volatility with a GARCH(1,1): after a market shock (a large price move), the model predicts elevated volatility over the following days (clustering), then a gradual decay toward long-run volatility — a behavior implicitly assumed by any desk that dynamically adjusts hedges after a shock, but that a constant-volatility model never captures.",
  },
  ...
  formula: {
    ...
    example: { fr: "ω=0,00001, α=0,08, β=0,90 : après un grand choc ε_{t-1}, σ_t² augmente immédiatement, puis décroît progressivement vers ω/(1−α−β) au fil des jours suivants sans nouveau choc.", en: "ω=0.00001, α=0.08, β=0.90: after a large shock ε_{t-1}, σ_t² rises immediately, then gradually decays toward ω/(1−α−β) over subsequent days absent a new shock." },
  },
  calculation: {
    fr: "1) Vérifier la stationnarité de la série (test de racine unitaire) ; différencier si nécessaire (ARIMA). 2) Ajuster les paramètres du modèle (AR/ARIMA sur la série elle-même, GARCH sur les résidus/rendements) par maximum de vraisemblance. 3) Valider chronologiquement : entraîner sur le passé, tester sur une période future jamais vue, jamais mélanger les dates. 4) Comparer à un modèle naïf (par exemple, volatilité constante égale à la moyenne historique) pour vérifier que la complexité additionnelle apporte un vrai gain prédictif.",
    en: "1) Check the series' stationarity (unit root test); difference if needed (ARIMA). 2) Fit the model's parameters (AR/ARIMA on the series itself, GARCH on residuals/returns) by maximum likelihood. 3) Validate chronologically: train on the past, test on a future period never seen, never mix dates. 4) Compare to a naive model (e.g., constant volatility equal to the historical average) to check the added complexity brings a real predictive gain.",
  },
  pythonExample: {
